            ...
            <div class="row">
                <div class="card rounded">
                    <div class="card-body">
                        ...
                        <form action="{{ route('syndic.update.cotisation', $cotisation->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            <div class="mb-3">
                                <label for="montant" class="form-label">Montant</label>
                                <input type="number" class="form-control @error('montant') is-invalid @enderror" id="montant" name="montant" value="{{ $cotisation->montant }}" required>
                                @error('montant')
                            ...
                            <div class="mb-3">
                                ...
                                <input type="date" class="form-control @error('date_cotisation') is-invalid @enderror" id="date_cotisation" name="date_cotisation" value="{{ $cotisation->date_cotisation }}" required>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control @error('description') is-invalid @enderror" id="description" name="description" value="{{ $cotisation->description }}" required>
                                @error('description')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            {{-- <div class="mb-3">
                                ...
                                <select class="form-control @error('appartement_id') is-invalid @enderror" id="appartement_id" name="appartement_id" required>
                                    @foreach($appartements as $appartement)
                                        <option value="{{ $appartement->id }}" {{ $appartement->id == $cotisation->appartement_id ? 'selected' : '' }}>{{ $appartement->nom_appartement }}</option>
                                    @endforeach
                                </select>
